...
current_users = ['ali', 'admin', 'hamza', 'usman', 'sherry'];
new_users = ['mahnoor', 'admin', 'hamza', 'ansar', 'komal'];

for (let i = 0; i < new_users.length; i++) {
    const newUser = (new_users[i]);
    for (let b = 0; b < current_users.length; b++) {
        const currentUsers = (current_users[b]);
        if (currentUsers.includes(newUser)) {
            console.log(`${newUser}, Username already use,Try another username.`);
